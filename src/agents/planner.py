...
class PlannerAgent(BaseAgent):
    ...
    def process(self, task: str) -> dict:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a planning agent. Output ONLY valid JSON containing a 'steps' array of strings. Do not include markdown formatting like ```json.",
            },
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                self.logger.info(
                    f"Attempt {attempt}/{self.max_retries} to generate plan..."
                )
                ...
                if attempt == self.max_retries:
                    ...
                    raise RuntimeError(
                        f"Planner failed after {self.max_retries} attempts."
                    ) from e
