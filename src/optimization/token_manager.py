...
class BaselinePipeline:
    ...
    def __init__(self):
        ...
        self.raw_tools = [
            {"user_id": i, "status": "active", "latency_ms": 120 + i}
            for i in range(200)
        ]
    ...
    def build_optimized_payload(self) -> list:
        ...
        hist_str = "\n".join(
            [f"{m['role']}: {m['content']}" for m in optimized_history]
        )
...
def run_analysis():
    ...
    print(
        "Quality Tradeoff: Minor history loss, but massive gain in 'Lost in the Middle' prevention."
    )
    print("=" * 60)
